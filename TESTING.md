...
另涵蓋 argv before／finally／success／failure action 的 timeout／required／cleanup
政策、failure reason 環境變數、live writer 降級、Btrfs pinned-FD 建立、
version-1 snapshot state machine、crash recovery、完整身分比對、orphan journal
maintenance、finally 清理與 capture inventory 驗證。
也涵蓋每個 action 強制 `run_as`、supplementary group 清除、root executable
信任檢查，以及 root staging／Docker volume helper 不降權，確保備份與還原能保存
數字 UID/GID、mode、ACL、xattr 與 hardlink。
