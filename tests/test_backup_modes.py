...
from homelab_backup import backup
...
class CaptureModeTests(unittest.TestCase):
    ...
    def common_patches(self):
        return (
            mock.patch.object(backup, 'cleanup_snapshot_state'),
            mock.patch.object(backup, 'validate_trusted_roots'),
            mock.patch.object(backup, 'ensure_private_directory', side_effect=self._ensure),
            mock.patch.object(backup, 'validate_docker_environment'),
            mock.patch.object(backup, 'validate_docker_bind_probe'),
            mock.patch.object(backup, 'compose_model', return_value={
                'name': 'demo', 'services': {'app': {}}, 'volumes': {},
            }),
            mock.patch.object(backup, 'validate_runtime_sources'),
            mock.patch.object(backup, 'resolved_volume_sources', return_value=[]),
            mock.patch.object(backup, 'compose_identity', return_value={
                'project_name': 'demo', 'services': ['app'],
                'compose_files': ['compose.yaml'], 'volumes': [],
            }),
            mock.patch.object(backup, 'validate_path_payloads'),
            mock.patch.object(backup, 'estimate_backup_size', return_value=0),
            mock.patch.object(backup, '_check_backup_space'),
            mock.patch.object(backup, 'atomic_copy_file', side_effect=shutil.copy2),
            mock.patch.object(backup, 'atomic_write_json', side_effect=self._write_json),
        )
    ...
    def test_live_writer_is_best_effort_and_optional_action_downgrades_inventory(self):
        ...
        with ExitStack() as stack:
            ...
            stack.enter_context(mock.patch.object(
                backup, 'run_before_actions', return_value=[{
                    'phase': 'before', 'name': 'checkpoint', 'result': 'failed',
                }],
            ))
            stack.enter_context(mock.patch.object(
                backup, 'docker_writer_maps',
                return_value=({'data': ('container',)}, {}),
            ))
            sync_paths = stack.enter_context(mock.patch.object(
                backup, 'sync_paths', return_value=[entry],
            ))
            stack.enter_context(mock.patch.object(
                backup, 'sync_volumes', return_value=[],
            ))
    ...
    def test_snapshot_override_is_staged_and_cleanup_runs_in_finally(self):
        ...
        with ExitStack() as stack:
            for patcher in patches:
                stack.enter_context(patcher)
            stack.enter_context(mock.patch.object(backup, 'cleanup_snapshot_state'))
            stack.enter_context(mock.patch.object(
                backup, 'run_before_actions', return_value=[],
            ))
            stack.enter_context(mock.patch.object(
                backup, 'run_finally_actions', side_effect=lambda _m: (
                    events.append('finally') or []
                ),
            ))
            stack.enter_context(mock.patch.object(
                backup, 'SnapshotTransaction', return_value=transaction,
            ))
            stack.enter_context(mock.patch.object(
                backup, 'docker_writer_maps', return_value=({}, {}),
            ))
            sync_paths = stack.enter_context(mock.patch.object(
                backup, 'sync_paths', return_value=[entry],
            ))
    ...
    def test_stale_snapshot_cleanup_runs_before_actions_in_non_snapshot_mode(self):
        ...
        with ExitStack() as stack:
            for patcher in patches:
                stack.enter_context(patcher)
            stack.enter_context(mock.patch.object(
                backup, 'cleanup_snapshot_state',
                side_effect=lambda _c, _service: events.append('cleanup'),
            ))
            stack.enter_context(mock.patch.object(
                backup, 'run_before_actions',
                side_effect=lambda _m: events.append('before') or [],
            ))
            stack.enter_context(mock.patch.object(
                backup, 'run_finally_actions', return_value=[],
            ))
            stack.enter_context(mock.patch.object(
                backup, 'validate_no_docker_writers',
            ))
            stack.enter_context(mock.patch.object(
                backup, 'sync_paths', return_value=[],
            ))
            stack.enter_context(mock.patch.object(
                backup, 'sync_volumes', return_value=[],
            ))
    ...
    def test_required_finally_failure_aborts_before_inventory_is_written(self):
        ...
        with ExitStack() as stack:
            ...
            stack.enter_context(mock.patch.object(
                backup, 'run_finally_actions', side_effect=RuntimeError('resume failed'),
            ))
            stack.enter_context(mock.patch.object(
                backup, 'validate_no_docker_writers',
            ))
    ...
    def test_finally_failure_does_not_replace_before_failure(self):
        ...
        with ExitStack() as stack:
            for patcher in patches:
                stack.enter_context(patcher)
            stack.enter_context(mock.patch.object(
                backup, 'run_before_actions', side_effect=before_failure,
            ))
    ...
    def test_finally_runs_when_docker_preflight_fails(self):
        ...
        with ExitStack() as stack:
            ...
            stack.enter_context(mock.patch.object(
                backup, 'run_finally_actions', finalizer,
            ))
            stack.enter_context(mock.patch.object(
                backup, 'compose_model', side_effect=RuntimeError('bad compose'),
            ))
